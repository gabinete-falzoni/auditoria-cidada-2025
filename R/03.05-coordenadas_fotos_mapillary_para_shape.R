...
# Mapillary: exiftool -filename -gpslatitude -gpslongitude -T -r . > "../mapillary_sequences_latlon.tsv"
...
pasta_fotos_mapillary_crop  <- sprintf('%s/05_mapillary_sequences_cropped', pasta_fotos_campo)
pasta_publicacao  <- sprintf('%s/07_publicacao', pasta_fotos_campo)
pasta_fotos_tsv   <- sprintf('%s/tsv_fotos', pasta_publicacao)
dir.create(pasta_fotos_mapillary_crop, recursive = TRUE, showWarnings = FALSE)
...
# pasta_fotos_campo <- '/mnt/fern/Dados/Campo_Camera360'
# pasta_fotos_campo_mapillary <- sprintf('%s/04_mapillary_sequences', pasta_fotos_campo)
# pasta_base  <- '/mnt/fern/Dados/gitlab/auditoria-cidada-2025/dados'
# pasta_publicacao <- sprintf('%s/dados_x_publicacao', pasta_base)


# ------------------------------------------------------------------------------
# Cortar e reduzir fotografias do Mapillary
# ------------------------------------------------------------------------------

# # Fotos das image_sequences mapillary, com coordenadas GPS já sincronizadas ao viário
# image_files <- data.frame(arqs = list.files(pasta_fotos_campo_mapillary,
#                                             pattern = '.*\\.jpg$',
#                                             recursive = TRUE,
#                                             full.names = TRUE))
#
# # As imagens do Mapillary estão com 5760x2880 pixels. Queremos a parte central
# # dela dividida horizontalmente em 4 (2880x2880). Vamos aproveitar para já
# # reduzir as imagens ao rodar o convert para fazer o crop.
# convert_path <- sprintf("/usr/bin/convert")
# for (i in image_files$arqs) {
#   # print(i)
#   # Atualizar timestamp a cada x segundos - -overwrite_original é opcional
#   # convert 20250414_071526_00014.jpg -crop 2880x2880+1440+0 +repage -resize 300x300 output.jpg
#   out_file <- sprintf('%s/%s', pasta_fotos_mapillary_crop, str_replace(basename(i), "\\.jpg", "_cropped.jpg"))
#   cmd <- sprintf("%s %s -crop 2880x2880+1440+0 +repage -resize 300x300 %s", convert_path, i, out_file)
...
# Entrar pelo terminal e rodar na pasta pasta_fotos_mapillary_crop:
# exiftool -filename -gpslatitude -gpslongitude -T -r . > "../mapillary_sequences_latlon.tsv"
...
fotos <- fotos %>% mutate(imagepath = str_c(pasta_fotos_mapillary_crop, X1, sep = '/'),
                          .before = 'lat')
# fotos %>% st_drop_geometry() %>% select(imagepath)

# URL para fotos no servidor: coluna foto_url, <img src='../fotos/2022-05-10_13-23-52.jpg'>
# https://falzoni.com.br/auditoria_cidada/fotos_mapillary_cropped/20250409_140416_00001_cropped.jpg
fotos <- fotos %>% mutate(foto_url = str_c("<img src='../fotos_mapillary_cropped/", X1, "'>", sep = ''), .after = X1)
...
out_file <- sprintf('%s/auditoria_cidada_2025_D_fotos_mapillary_cropped.gpkg', pasta_publicacao)
