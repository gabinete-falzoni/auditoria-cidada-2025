...
out_mp4 <- sprintf('%s/tmp_video.mp4', pasta_dados)
...
arg_o3 <- sprintf('-filter_complex "[0:v][1:v]hstack=inputs=2" -c:v libx264 -crf 10 -preset fast -y %s', out_mp4)
...
arg_o2 <- sprintf('-vf v360=dfisheye:e:yaw=0:ih_fov=%s:iv_fov=%s -q:v 3', fov, fov)
...
out_file <- sprintf('%s/gpx_para_revisao.gpkg', pasta_dados)
